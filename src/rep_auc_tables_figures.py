...
def calculate_pretrain_acc_AUC(home, source_data, target_data, augment, batch_size, img_length, img_width):
    ...
    trained_model = load_model(f'{home}/trained_models/model_weights_resnet_pretrained={source_data}.h5')
    print('found model')
    ...
    for train_index, val_index in skf.split(np.zeros(len(dataframe)), y=dataframe[['class']]):
        ...
        try:
            trained_model = load_model(
                f'{home}/trained_models/model_weights_resnet_target='
...
def create_AUC_matrix(home, x_col, y_col, augment, k, batch_size):
    ...
    source_datasets = ['imagenet', 'stl10', 'textures', 'isic', 'chest', 'pcam-middle', 'pcam-small', 'kimia']
...
def create_heatmap(mean_auc_dict, overall_ranking):
    ...
    print (np.arange(auc_matrix_sorted.shape[0] - 2))
    print ('auc_matix_sorted', auc_matrix_sorted)
    print ('auc_matrix_nonan', auc_matrix_nonan)
    ...
    #ax.set_yticklabels(['ImageNet', 'STL-10', 'STI-10', 'DTD', 'ISIC2018', 'Chest X-rays', 'PCam-middle', 'PCam-small','KimiaPath960'])
    ax.set_yticklabels(['ImageNet', 'STL-10', 'DTD', 'ISIC2018', 'Chest X-rays', 'PCam-middle', 'PCam-small','KimiaPath960'])
...
def create_barplot(mean_auc_dict, std_auc_dict):
    ...
    #source = ['ImageNet', 'STL-10', 'STI-10', 'DTD', 'ISIC2018', 'Chest X-rays', 'PCam-middle', 'PCam-small', 'KimiaPath960']
    source = ['ImageNet', 'STL-10', 'DTD', 'ISIC2018', 'Chest X-rays', 'PCam-middle', 'PCam-small', 'KimiaPath960']
